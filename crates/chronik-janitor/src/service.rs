...
use chronik_common::metadata::{MetadataStore, FileMetadataStore};
...
impl JanitorService {
    ...
    pub async fn new(config: JanitorConfig) -> Result<Self> {
        ...
        // Create metadata store using FileMetadataStore
        let metadata_path = std::path::PathBuf::from(&config.metadata_path);
        let metadata_store = Arc::new(FileMetadataStore::new(metadata_path).await?) as Arc<dyn MetadataStore>;
