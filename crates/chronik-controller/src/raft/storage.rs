//! Persistent storage backend for Raft using Sled.
...
const SNAPSHOT_KEY: &[u8] = b"current_snapshot";
...
impl RaftStorage {
    ...
    /// Initialize with configuration state (for new clusters)
    pub fn initialize_with_conf_state(&self, voters: Vec<u64>, learners: Vec<u64>) -> Result<()> {
        ...
        // Create initial empty entry at index 0
    ...
    /// Truncate log entries from index
    ...
    pub fn create_snapshot(&self, index: u64, term: u64, nodes: ConfState, data: Vec<u8>) -> Result<()> {
        ...
        self.snapshot_tree.insert(SNAPSHOT_KEY, snapshot_data)
    ...
    pub fn snapshot(&self) -> Result<Snapshot> {
        if let Some(data) = self.snapshot_tree.get(SNAPSHOT_KEY)
...
// Wrapper to implement Storage for Arc<RaftStorage>
pub struct ArcRaftStorage(pub Arc<RaftStorage>);

impl Storage for ArcRaftStorage {
    fn initial_state(&self) -> raft::Result<RaftState> {
        self.0.initial_state().map_err(|e| raft::Error::Store(raft::StorageError::Other(Box::new(e))))
    }
    
    fn entries(&self, low: u64, high: u64, max_size: impl Into<Option<u64>>, context: GetEntriesContext) -> raft::Result<Vec<Entry>> {
        self.0.entries(low, high, max_size, context).map_err(|e| raft::Error::Store(raft::StorageError::Other(Box::new(e))))
    }
    
    fn term(&self, idx: u64) -> raft::Result<u64> {
        self.0.term(idx).map_err(|e| raft::Error::Store(raft::StorageError::Other(Box::new(e))))
    }
    
    fn first_index(&self) -> raft::Result<u64> {
        self.0.first_index().map_err(|e| raft::Error::Store(raft::StorageError::Other(Box::new(e))))
    }
    
    fn last_index(&self) -> raft::Result<u64> {
        self.0.last_index().map_err(|e| raft::Error::Store(raft::StorageError::Other(Box::new(e))))
    }
    
    fn snapshot(&self, request_index: u64, to: u64) -> raft::Result<Snapshot> {
        let _request_index = request_index;
        let _to = to;
        self.0.snapshot().map_err(|e| raft::Error::Store(raft::StorageError::Other(Box::new(e))))
    }
}
