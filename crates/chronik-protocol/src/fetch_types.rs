...
impl KafkaDecodable for FetchRequestPartition {
...
impl KafkaEncodable for FetchRequestPartition {
...
impl KafkaDecodable for FetchRequestTopic {
    fn decode(decoder: &mut Decoder, version: i16) -> Result<Self> {
        ...
        for _ in 0..partition_count {
            partitions.push(FetchRequestPartition::decode(decoder, version)?);
        }
...
impl KafkaEncodable for FetchRequestTopic {
    fn encode(&self, encoder: &mut Encoder, version: i16) -> Result<()> {
...
pub struct FetchRequest {
    ...
    pub max_wait_ms: i32,
    /// Min bytes to accumulate before returning
    pub min_bytes: i32,
...
impl KafkaDecodable for FetchRequest {
    fn decode(decoder: &mut Decoder, version: i16) -> Result<Self> {
        ...
        for _ in 0..topic_count {
            topics.push(FetchRequestTopic::decode(decoder, version)?);
        }
...
impl KafkaEncodable for AbortedTransaction {
...
impl KafkaEncodable for FetchResponsePartition {
    fn encode(&self, encoder: &mut Encoder, version: i16) -> Result<()> {
        encoder.write_i32(self.partition);
...
pub struct FetchResponse {
    ...
    /// Error code (v7+, for session errors)
    pub error_code: i16,
    /// Session ID (v7+)
    pub session_id: i32,
    /// Topic responses
