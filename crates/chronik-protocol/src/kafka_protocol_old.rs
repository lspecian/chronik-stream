//! Self-contained Kafka protocol implementation without external dependencies.
//! 
//! This module provides a complete implementation of the Kafka wire protocol
//! that is fully compatible with official Kafka clients.
...
/// Protocol decoder for compact types
pub trait CompactDecoder: Buf {
    /// Read a compact string
    fn read_compact_string(&mut self) -> Result<Option<String>> {
    ...
    fn read_compact_array_len(&mut self) -> Result<usize> {
        let len = self.read_unsigned_varint()? as i32 - 1;
        if len < 0 {
            Ok(0)
        } else {
    ...
    fn read_tagged_fields(&mut self) -> Result<()> {
        let num_fields = self.read_unsigned_varint()?;
...
impl<T: Buf> CompactDecoder for T {}

/// Protocol encoder for compact types
pub trait CompactEncoder: BufMut {
    /// Write a compact string
    fn write_compact_string(&mut self, value: Option<&str>) {
        match value {
            Some(s) => {
    ...
    fn write_compact_array_len(&mut self, len: usize) {
        self.write_unsigned_varint((len + 1) as u32);
    }
    
    /// Write an unsigned varint
    fn write_unsigned_varint(&mut self, mut value: u32) {
        while (value & !0x7F) != 0 {
            self.put_u8((value & 0x7F) as u8 | 0x80);
            value >>= 7;
        }
    ...
    fn write_tagged_fields(&mut self) {
        self.write_unsigned_varint(0);
    }
}

impl<T: BufMut> CompactEncoder for T {}
...
impl RequestHeader {
    ...
    pub fn encode(&self, buf: &mut dyn BufMut, flexible_version: bool) {
        ...
        if flexible_version {
            buf.write_compact_string(self.client_id.as_deref());
            buf.write_tagged_fields();
        } else {
            // Standard string
...
impl ProtocolCodec {
    ...
    pub fn encode_api_versions_response(
        ...
        // API versions array
        if flexible {
            buf.write_compact_array_len(api_versions.len());
        } else {
    ...
    pub fn encode_metadata_response(
        ...
        // Brokers array
        if flexible {
            buf.write_compact_array_len(brokers.len());
        } else {
        ...
        for (node_id, host, port) in brokers {
            ...
            if flexible {
                buf.write_compact_string(Some(host));
            } else {
        ...
        if version >= 2 {
            if flexible {
                buf.write_compact_string(Some(cluster_id));
            } else {
        ...
        // Topics array
        if flexible {
            buf.write_compact_array_len(topics.len());
        } else {
        ...
        for topic in topics {
            ...
            // Topic name
            if flexible {
                buf.write_compact_string(Some(&topic.name));
            } else {
            ...
            // Partitions array
            if flexible {
                buf.write_compact_array_len(topic.partitions.len());
            } else {
            ...
            for partition in &topic.partitions {
                ...
                // Replicas
                if flexible {
                    buf.write_compact_array_len(partition.replicas.len());
                } else {
                ...
                // ISR
                if flexible {
                    buf.write_compact_array_len(partition.isr.len());
                } else {
                ...
                if version >= 5 {
                    if flexible {
                        buf.write_compact_array_len(partition.offline_replicas.len());
                    } else {
...
mod tests {
    ...
    fn test_varint_encoding() {
        ...
        buf.write_unsigned_varint(0);
        buf.write_unsigned_varint(127);
        buf.write_unsigned_varint(128);
        buf.write_unsigned_varint(16383);
        buf.write_unsigned_varint(16384);
        
        let mut frozen = buf.freeze();
        assert_eq!(frozen.read_unsigned_varint().unwrap(), 0);
        assert_eq!(frozen.read_unsigned_varint().unwrap(), 127);
        assert_eq!(frozen.read_unsigned_varint().unwrap(), 128);
        assert_eq!(frozen.read_unsigned_varint().unwrap(), 16383);
        assert_eq!(frozen.read_unsigned_varint().unwrap(), 16384);
    }
    ...
    fn test_compact_string() {
        ...
        buf.write_compact_string(Some("hello"));
        buf.write_compact_string(None);
        buf.write_compact_string(Some(""));
        
        let mut frozen = buf.freeze();
        assert_eq!(frozen.read_compact_string().unwrap(), Some("hello".to_string()));
        assert_eq!(frozen.read_compact_string().unwrap(), None);
        assert_eq!(frozen.read_compact_string().unwrap(), Some("".to_string()));
    }
