...
// Test mode: text-ceiling, vector, hybrid, mixed (default)
const TEST_MODE = __ENV.TEST_MODE || 'mixed';

// VU stages depend on test mode
...
function hybridSearch() {
  const q = queries[Math.floor(Math.random() * queries.length)];
  const k = Math.random() < 0.7 ? 5 : 10;
  const vw = 0.5 + Math.random() * 0.4;  // 0.5-0.9
...
export default function () {
  switch (TEST_MODE) {
    ...
    case 'hybrid':
      hybridSearch();
      break;
    default: // mixed: 60% text, 20% vector, 20% hybrid
      const r = Math.random();
      if (r < 0.6) textSearch();
