...
use chronik_storage::{ObjectStoreConfig, ObjectStoreBackend, OpenDalObjectStore, ObjectStore};
use bytes::BytesMut;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 2: Storage
    println!("\n2. Testing Storage:");
    let storage_config = ObjectStoreConfig {
        backend: ObjectStoreBackend::Local {
            path: "/tmp/chronik-test".to_string(),
        },
    ...
    match OpenDalObjectStore::new(storage_config).await {
        Ok(storage) => {
            // Test write
