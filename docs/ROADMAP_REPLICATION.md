...
- **A topic's partition count can disagree between nodes — and this probably *causes* the above.** A topic created with `--partitions 1` was later reported by `/admin/status` as having three, with real assignments and leaders for partitions 1 and 2, while every record sat in partition 0 and only partition 0 existed on disk. The producer had clearly seen one partition when it wrote.
  ...
  Not reproduced by a plain node restart (a 1-partition topic stayed at 1 across one). Both anomalies appeared while the cluster was in flux from cordon/uncordon. **Next step**: create a 1-partition topic and compare each node's `/admin/status` view directly — if followers report three while the leader reports one, it is in the metadata replication path. The hardcoded `partition_count: 3` in `TopicConfig::default()` (`traits.rs:40`) is the obvious candidate for something reconstructing a config it should have read; the other hardcoded `3`s nearby are test-only.
